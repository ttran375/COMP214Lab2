...
SELECT INITCAP(LastName) || ' ' || INITCAP(FirstName) AS CustomerName
FROM Customers;

-- 2. Create a list of all customer numbers along with text indicating whether 
-- the customer has been referred by another customer. Display the text 
-- “NOT REFERRED” if the customer wasn’t referred to JustLee Books by another 
-- customer or “REFERRED” if the customer was referred.
SELECT Customer#,
  CASE
...
-- 3. Determine the amount of total profit generated by the book purchased on 
-- order 1002. Display the book title and profit. The profit should be 
-- formatted to display a dollar sign and two decimal places. Take into account 
-- that the customer might not pay the full retail price, and each item ordered 
...
-- 4. Display a list of all book titles and the percentage of markup for each 
-- book. The percentage of markup should be displayed as a whole number (that 
-- is, multiplied by 100) with no decimal position, followed by a percent sign 
-- (for example, .2793 = 28%). (The percentage of markup should reflect the 
...
SELECT TO_CHAR(SYSDATE, 'Day') AS DayOfWeek,
  TO_CHAR(SYSDATE, 'HH24') AS Hour,
  TO_CHAR(SYSDATE, 'MI') AS Minutes,
  TO_CHAR(SYSDATE, 'SS') AS Seconds
FROM dual;

-- 6. Create a list of all book titles and costs. Precede each book’s cost with 
-- asterisks so that the width of the displayed Cost field is 12.
SELECT Title,
  RPAD('*', 12 - LENGTH(TO_CHAR(Cost)), '*') || TO_CHAR(Cost, '999.99') AS 
  FormattedCost
FROM Books;
...
-- portions of months. Display the book title, publication date, current date, 
-- and age.
SELECT Title,
...
-- of his or her zip code. The query should also display the position of the 
-- first occurrence of a 3 in the customer number, if it exists.
SELECT Customer#,
