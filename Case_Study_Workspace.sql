-- **Case Study** Note: Make sure you have run the CityJail_8.sql script from 
-- Chapter 8. This script makes all database objects available for completing 
-- this case study. The following list reflects current data requests from city 
-- managers. Provide the SQL statement to satisfy each request. Test the 
-- statements and show execution results.
-- 1.  List the following information for all crimes that have a period greater 
-- than 14 days between the date charged and the hearing date: crime ID,  
-- classification, date charged, hearing date, and number of days between the  
-- date charged and the hearing date.
SELECT
  c.crime_id,
  c.classification,
  c.date_charged,
...
-- community assignment, indicated by the second letter of the precinct code. 
-- Display the community description listed in the following chart, based on 
-- the second letter of the precinct code.
SELECT
  o.officer_id,
...
-- criminal name, charge ID, total amount owed (fine amount plus court fee), 
-- amount paid, amount owed, and payment due date. If nothing has been paid to 
-- date, the amount paid is NULL. Include only criminals who owe some amount 
-- of money. Display the dollar amounts with a dollar sign and two decimals.
SELECT
  UPPER(c.last) || ', ' || UPPER(c.first) AS criminal_name,
  cc.charge_id,
...
-- 5.  Display the criminal name and probation start date for all criminals who 
-- have a probation period greater than two months. Also, display the date 
-- that’s two months from the beginning of the probation period, which will 
-- serve as a review date.
SELECT
  UPPER(c.last) || ', ' || UPPER(c.first) AS criminal_name,
...
-- Create an INSERT statement using substitution variables. Note that users 
-- will be entering dates in the format of a two-digit month, a two-digit day, 
-- and a four-digit year, such as “12 17 2009.” In addition, a sequence named 
-- APPEALS_ID_SEQ exists to supply values for the Appeal_ID column, and the 
-- default setting for the Status column should take effect (that is, the 
-- DEFAULT option on the column should be used). Test the statement by adding 
-- the following appeal: crime_ID ¼ 25344031, filing date ¼ 02 13 2009, and 
